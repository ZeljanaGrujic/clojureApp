...
(defn index [orders]
  (if (= orders ())
    (html5 [:h1 "Jos uvek nema kreiranih narudzbina"])
...
(defn num-order-per-person [{full_name :full_name maked_orders :maked_orders total_amount :total_amount total_price :total_price}]
  (html5
    [:li (format " Narucilac: %s            Broj naruzbina: %s       Ukupna kolicina: %s komada             Ukupna cena: %s dinara" full_name maked_orders total_amount total_price)]))
...
(defn order-view-session [{full_name :full_name do_date :do_date street :street amount_id :amount_id user_id :user_id package_name :package_name price :price}]
  (html5
    [:li (format " Ime na koje se vrsi isporuka: %s               Datum isporuke: %s                  Ulica: %s         Kolicina: %s                Cena: %s  dinara" full_name do_date street package_name price)]))
...
(defn total-orders-per-month [{month_name :month_name monthly_orders :monthly_orders total_price :total_price total_amount :total_amount}]
  (html5
    [:li (format " Mesec: %s            Broj narudzbina: %s        Ukupna cena: %s evra           Ukupna kolicina: %s kg    " month_name monthly_orders total_price total_amount)]))
...
(defn base-statistic-page []
  (base
    ...
    [:h4 (format "Ukupan broj ostavrenih narudzbina: %s" (- (odb/total-num-orders) 2))]
    [:h4 (format "Ukupan broj isporucenih narudzbina: %s" (odb/total-num-delivered-orders))]
    [:h4 (format "Ukupan broj neisporucenih narudzbina: %s" (- (odb/total-num-undelivered-orders) 2))]
    [:h4 (format "Ukupno ostvareni prihod: %s dinara" (- (odb/total-income) 260))]
    [:hr]
  ...
  (html5
    [:body
     (form/form-to [:post (str "/food-order/new/" (fodb/get-next-food-id))]
                   ...
                   (form/submit-button "Sacuvaj narudzbinu"))]))
