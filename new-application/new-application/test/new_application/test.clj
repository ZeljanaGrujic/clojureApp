...
(deftest test-get-id-by-type-order
  ...
  (testing "id-by-order-type"
    (let [res (odb/get-id-by-type-order "40komada")]
      (is (= (integer? (odb/get-id-by-type-order "40komada")) true))
      (is (> res 0))
      (is (= res 4)))))
...
(deftest test-create-new-order
      ...
      (testing "create order"
        (let [succes (odb/new-order {:full_name "Test order" :do_date "00.00.0000." :city_part "HRS" :street "Street test" :delivered "NE" :package_name "10komada" :phone "060-0000-000"})]
          (is (= succes (seq '(1)))))))

(deftest test-edit-order
  "If editing oder is successful it will return (1) in sequence"
  (testing "edit order"
    (let [succes (odb/edit-order {:id 10 :full_name "Test order" :do_date "00.00.0000." :city_part "HRS" :street "Street test" :delivered "NE" :package_name "10komada"})]
      (is (= succes (seq '(1)))))))
...
(deftest test-get-order-by-id1
  "It will return test order which has id 12, this result is taken for testing needs, from database
  If anything is changed to this order with id 12, it sholud also be changed in this test"
  (testing "get order by id"
    (let [succes (odb/get-order-by-id1 12)]
      (is (= succes {:amount 10,
                     :city_part "HRS",
                     :phone "060-0000-000",
                     :delivered "NE",
...
(deftest test-order-by-name
  ...
  (testing "order by name"
    (let [succes (odb/get-order-by-name "Test order")]
      (is (or (= (empty? succes) true) (= (empty? succes) false))))))



;RECOMMENDATION - do not run this test, trust me it works properly
(deftest test-delete-order
  ...
  (testing "delete order"
    (let [succes (odb/delete-order {:id 12, :full_name "Test order", :amount "10", :do_date "00.00.0000.", :location "HRS" :delivered "NE"})]
      (is (= succes (seq '(1)))))))
...
(deftest test-create-food-order
  ...
  (testing "create new food order"
    (let [succes (fodb/new-food-order {:do_date "19.1.2023." :month_name "Januar" :type_name "Pantelic zito"})]
      (is (= succes (seq '(1)))))))
...
(deftest test-get-food-order-by-id
  "It will return test order which has id 1, this result is taken for testing needs, from database
  If anything is changed to this order with id 1, it sholud also be changed in this test"
  (testing "get food order by id"
    (let [succes (fodb/get-food-order-by-id 1)]
      (is (= succes {:id 1, :amount 1000.0, :do_date "9.1.2023.", :month_name "Januar", :type_id 1})))))
...
(deftest test-delete-food-order
  ...
  (testing "delete food order"
    (let [succes (fodb/delete-food-order {:id 1, :amount 1000.0, :do_date "9.1.2023.", :month_name "Januar", :type_id 1})]
      (is (= succes (seq '(1)))))))
...
(deftest test-if-exist
  ...
  (testing "exist"
    (let [succes (odb/if-exist {:owner_name "Biljana" :owner_surname "Grujic" :phone "060-0133-611" :password "bilja"})]
      (is (or (= (empty? succes) true) (= (empty? succes) false))))))
...
(deftest test-users-orders-session
  ...
  (testing "session-orders"
    (let [succes (odb/user-orders-session 1)]
      (is (or (= (empty? succes) true) (= (empty? succes) false))))))
...
(deftest test-create-user-note
  ...
  (testing "create new user note"
    (let [succes (odb/create-note {:message_body "Test note" :user_id 10})]
      (is (= succes (seq '(1)))))))
...
(deftest test-edit-note
  "If editing note is successful it will return (1) in sequence"
  (testing "edit note"
    (let [succes (odb/edit-note {:message_body "Test note" :id 4 :read "NE"})]
      (is (= succes (seq '(1)))))))